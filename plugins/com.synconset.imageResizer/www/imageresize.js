...
/**
 * Set stored image as wallpaper
 * @param success success callback, will receive the data sent from the native plugin
 * @param fail error callback, will receive an error string describing what went wrong
 * @param filename : whether morning, afternoon or night
 * @param directory : directory relative to temporary directory of the app to store image
 * @param options extra options -  
 *              
 * @returns JSON Object with the following parameters:
 *              url : URL of the file set as wallpaper
 */
ImageResizer.prototype.setWallpaper = function(success, fail, filename, directory) {
    ...
    return cordova.exec(success, fail, "ImageResizePlugin", "setWallpaper", [params]);
};
